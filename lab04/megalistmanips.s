...
map:
    ...
    add t3, x0, t0      # FIXED 1: t3 is offset of array
...
mapLoop:
    ...
    # FIXED 3: offset is a word length 
    add t3, t0, x0
    slli t3, t3, 2
    add t1, t1, t3      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0

    # FIXED 4: save all temp registers and ra
    addi sp, sp, -20
    sw t3, 0(sp)
    sw t2, 4(sp)
    sw t1, 8(sp)
    ...
    # FIXED 4: restore all temp register and ra
    lw t3, 0(sp)
    lw t2, 4(sp)
    lw t1, 8(sp)
    lw t0, 12(sp)
